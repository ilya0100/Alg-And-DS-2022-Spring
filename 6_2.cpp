...
size_t partition(T* arr, size_t l, size_t r, Compare cmp = IsLessDefault<T>()) {
    ...
    std::swap(arr[median_of_three(arr, l, (l + r) / 2, r, cmp)], arr[l]);
    T pivot = arr[l];
    size_t i = r - 1;
    size_t j = i;
...
template <class T>
T find_k_stat(T* arr, size_t n, size_t k) {
    size_t l = 0;
    size_t r = n;
    size_t pivot_pos;
...
void run(std::istream& input, std::ostream& output) {
    ...
    size_t percentile_10 = (n + 9) / 10;
    size_t percentile_90 = n < 10 ? n : n / 10 * 9;
    size_t median = n / 2;
...
void run_tests() {
    {
        int a[] = {1, 2, 3};
        assert(median_of_three(a, 0, 1, 2) == 1);
        assert(median_of_three(a, 1, 0, 2) == 1);
        assert(median_of_three(a, 0, 2, 1) == 1);
    }
    {
